...
같은 고객 문의에 프롬프트를 바꿔 적용하고, **답변의 어떤 부분이 맞고 틀렸는지 직접 확인하는** 2주차 실습입니다. 작년 4개 노트북의 문제 정의, 출력 형식, 소수 예시, 업무 분기, 오류 처리를 하나의 고객지원 업무로 연결했습니다. 원본은 `AS-IS 2025 2nd semester/practice/week02/`에 그대로 보존합니다.
...
| 시간 | 화면 / 작업 | 확인할 내용 |
|---|---|---|
| 0–7분 | 실험 설정 · Demo 실행 | 합성 문의 8건, 동일한 입력, 실제 모델이 아닌 고정 응답 |
| 7–17분 | 프롬프트 확인 · v1/v2/v3 비교 | v2의 출력 형식과 업무 규칙, v3의 별도 예시 3건 |
| 17–30분 | 실패 살펴보기 · 결과 상세 | T01 형식 실패, T03 분류 오류, T08 미확정 환불 약속 |
| 30–38분 | 선택 이유 기록 · JSON 저장 | 개선 근거, 남은 한계, 다음 평가 사례 |
...
이번 2주차부터 격주 과제가 시작됩니다. 출제 후 일주일 이내 제출하며, 이번 과제는 **2026년 9월 18일 이내**에 제출합니다. 마감 시각은 별도 공지를 확인합니다.

실습에서 내보낸 JSON 보고서와 짧은 분석 PDF를 제출합니다. PDF에는 **실패 1건, 프롬프트 선택 근거, 새 합성 문의 1건과 기대 기준, 교수에게 묻고 싶은 질문**을 담습니다. Demo만으로 필수 과제를 끝낼 수 있으며 실제 모델 실행은 선택입니다. 새 문의를 실제로 실행하지 않았다면 “검증 계획 · 미실행”으로 구분합니다.

[과제 안내의 제출 경로와 체크리스트](../ASSIGNMENT_01.md)를 확인하세요. 제출물은 소속별 LMS·이메일로 제출하며 공개 GitHub에 올리지 않습니다. 이메일 주소는 수업에서 별도 안내합니다. 실제 고객 정보·학생 개인정보·API 키·접속 토큰은 실험 데이터와 보고서에 넣지 않습니다.
