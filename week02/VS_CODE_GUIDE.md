...
1. STEP 01: demo와 8건 선택 확인 → **3개 버전 비교 실행**.
2. STEP 02: v1, v2, v3를 차례로 선택 → 시스템 메시지와 사용자 문의 비교.
3. STEP 03: T01/v1, T03/v2, T08/v2의 결과 버튼 → 원문, 기대 기준, 실패 원인 확인.
4. STEP 04: 다음 검증 후보 선택 → 근거 15자 이상 입력 → **결정 기록 추가**.
5. **JSON 보고서 내보내기** → 다운로드된 파일을 VS Code에서 열어 해시·설정·검사·결정 기록 확인.
