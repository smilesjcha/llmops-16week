...
- 8주차는 `기말 프로젝트 기획서 0.1 제출`, 16주차는 `기말 프로젝트 발표`라는 검정 이정표로 유지한다. 중간 또는 마지막 위치라는 이유만으로 파란색을 적용하지 않는다.
- 4주차 `2026.09.25 · 추석`과 6주차 `2026.10.09 · 한글날`에는 `온라인 강의영상 업로드`를 날짜와 함께 명시한다. 두 주차를 `휴강`으로 표기하지 않는다.
